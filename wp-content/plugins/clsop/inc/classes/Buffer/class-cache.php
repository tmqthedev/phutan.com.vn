...
class Cache extends Abstract_Buffer {
	...
	public function maybe_init_process() {
		if ( ! $this->tests->can_init_process() ) {
			...
			$this->comment_last_test_error(); // CL.
			return;
		}
	...
	public function maybe_process_buffer( $buffer ) {
		if ( ! $this->tests->can_process_buffer( $buffer ) ) {
			$this->log_last_test_error();
			$this->comment_last_test_error( $buffer ); // CL.
			return $buffer;
		}
	...
	/**
	 * Generates the referrer string to be used in URLs.
	 *
	 * @return string Referrer string to be used in URLs.
	 * @since 3.15.4-1.1-13
	 * @note CL
	 */
	public static function get_referrer(): string {
		...
		return '&_wp_http_referer=' . rawurlencode( remove_query_arg( 'fl_builder', $uri ) );
	}
}

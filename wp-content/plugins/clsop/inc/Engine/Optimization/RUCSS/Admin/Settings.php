...
class Settings {
	...
	public function display_success_notice() {
		...
		$message = sprintf(
			...
			__( '%1$s: The Used CSS of your homepage has been processed. AccelerateWP will continue to generate Used CSS for up to %2$s URLs per %3$s second(s).', 'rocket' ),
			'<strong>AccelerateWP</strong>',
	...
	public function display_wrong_license_notice() {
		...
		$main_message = __( "We couldn't generate the used CSS because you're using a nulled version of AccelerateWP. You need an active license to use the Remove Unused CSS feature and further improve your website's performance.", 'rocket' );
		$cta_message  = sprintf(
			// translators: %1$s = promo percentage.
			__( 'Click here to get a AccelerateWP single license at %1$s off!', 'rocket' ),
			'10%%'
		);
		...
		// translators: %1$s = plugin name, %2$s = opening anchor tag, %3$s = closing anchor tag.
			...
			'<strong>AccelerateWP</strong>',
			'<a href="https://cloudlinux.com/?add-to-cart=191&coupon_code=iamnotapirate10" class="button button-primary" rel="noopener noreferrer" target="_blank">',
			'</a>'
		);
	...
	public function display_saas_error_notice() {
		...
		// CL.
		$main_message = __( 'It seems a security plugin or the server\'s firewall prevents AccelerateWP from accessing the Remove Unused CSS generator.', 'rocket' );
		...
		$message = "<strong>AccelerateWP</strong>: $main_message<ul><li>$security_message</li><li>$firewall_message</li></ul>";
	...
	public function display_no_table_notice() {
		...
		$main_message = __( 'Could not create the %2$s table in the database which is necessary for the Remove Unused CSS feature to work.', 'rocket' );
	...
	protected function get_support_url() {
		return rocket_get_external_url(
			...
			[
				...
				'utm_medium' => 'cloudlinux_site_optimization_plugin',
			]
		);
	}
}

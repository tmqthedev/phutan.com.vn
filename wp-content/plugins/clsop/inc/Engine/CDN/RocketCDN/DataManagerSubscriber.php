...
class DataManagerSubscriber implements Subscriber_Interface {
	...
	public function sanitize_options( $input, $settings ) : array {
		$awp_account_id = $this->cdn_options->get_awp_account_id();
		$awp_cdn_url    = $this->cdn_options->get_awp_cdn_url();

		if ( ! empty( $awp_account_id ) && ! empty( $awp_cdn_url ) ) {
			$input['cdn'] = 1;
		}

		$input['cdn_awp_account_id'] = $awp_account_id;
		$input['cdn_awp_cdn_url']    = $awp_cdn_url;
		$input['cdn_awp_api_key']    = $this->cdn_options->get_awp_cdn_api_key();

...
class Tests {
	...
	public function can_init_process() {
		...
		// Don't process if file DONOTCACHEPAGE exists in the wp-content folder.
		$stop_file = dirname( __DIR__, 5 ) . DIRECTORY_SEPARATOR . 'DONOTCACHEPAGE';
	...
	public function can_process_buffer( $buffer ) {
		...
		if ( ! function_exists( 'rocket_mkdir_p' ) ) {
			// Uh?
			$this->set_error( 'AccelerateWP not found - page cannot be cached.' );
			return false;
		}
	...
	public function is_speed_tool() {
		...
		$user_agent = preg_match( '#PingdomPageSpeed|DareBoost|Google|PTST|Chrome-Lighthouse|AccelerateWP#i', $this->config->get_server_input( 'HTTP_USER_AGENT' ) );

...
class Detect_Missing_Tags_Subscriber implements Subscriber_Interface {
	...
	public function rocket_notice_missing_tags() {
		...
		if ( ! current_user_can( 'rocket_manage_options' ) || 'settings_page_clsop' !== $screen->id ) {
			return;
		}
		...
		$msg  = '<b>' . __( 'AccelerateWP: ', 'rocket' ) . '</b>';
		$msg .= sprintf(
		...
		$msg .= ' ' . sprintf(
			...
			'<a href="' . esc_url( __( 'https://docs.cloudlinux.com/article/99-pages-not-cached-or-minify-cssjs-not-working/?utm_source=wp_plugin&utm_medium=cloudlinux_site_optimization_plugin#theme', 'rocket' ) ) . '" rel="noopener noreferrer" target="_blank">',
			'</a>'
		);
		*/

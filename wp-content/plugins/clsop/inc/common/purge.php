...
/**
 * Clears all cache domain.
 *
 * @param string $lang Optional. Language code. Default empty.
 *
 * @return bool
 *
 * @note CL
 * @since 3.13.4_1.1-8
 */
function rocket_purge_all_cache_domain( $lang = '' ) {
	...
	if ( get_rocket_option( 'manual_preload' ) && ( ! defined( 'WP_ROCKET_DEBUG' ) || ! WP_ROCKET_DEBUG ) ) {
		...
		$args = (array) apply_filters(
			...
			[
				'blocking'   => false,
				'timeout'    => 0.01,
				'user-agent' => 'AccelerateWP/Homepage_Preload_After_Purge_Cache',
	...
	if ( isset( $_GET['type'], $_GET['_wpnonce'] ) ) {
		...
		switch ( $type ) {
			...
			case 'all':
				...
				rocket_purge_all_cache_domain( $lang );
				break;

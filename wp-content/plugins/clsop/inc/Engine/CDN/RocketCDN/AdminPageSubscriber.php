...
class AdminPageSubscriber extends Abstract_Render implements Subscriber_Interface {
	...
	public function rocketcdn_field( $fields ) {
		...
		// CL start.
		$fields['cdn']['description']            = Page::get_disable_feature_text( 'cdn' );
		$fields['cdn']['input_attr']['disabled'] = 1;
		$fields['cdn']['container_class'][]      = 'wpr-isDisabled';
		// CL end.

		$helper_text = __( 'Your AccelerateWP subscription is currently active.', 'rocket' );
		...
		if ( empty( $cdn_cnames ) || $cdn_cnames[0] !== $subscription_data['cdn_url'] ) {
			$helper_text = sprintf(
				...
				__( 'To use CDN, replace your CNAME with %1$s%2$s%3$s.', 'rocket' ),
				'<code>',
		...
		$fields['cdn_cnames'] = [
			...
			'helper'      => '', // Temporary off $helper_text . ' ' . $more_info.
			'default'     => '',
	...
	public function add_subscription_modal() {
		...
		$iframe_src = add_query_arg(
				[
					'website'  => home_url(),
					'callback' => rest_url( 'clsop/v1/rocketcdn/' ),
				],

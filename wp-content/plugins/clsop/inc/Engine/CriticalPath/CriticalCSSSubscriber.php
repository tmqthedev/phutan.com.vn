...
class CriticalCSSSubscriber implements Subscriber_Interface {
	...
	public static function get_subscribed_events() {
		...
		return [
			...
			'rocket_input_sanitize' => [ 'sanitize_options', 16, 2 ],
			'rocket_before_rollback' => [ 'stop_critical_css_generation', 9 ],
			'wp_rocket_upgrade' => [ 'stop_critical_css_generation', 9 ],
			'rocket_cpcss_accelerate_wp_cli_disable' => 'stop_process_on_cli_deactivation',
	...
	public function notice_critical_css_generation_triggered() {
		...
		if ( current_user_can( 'rocket_manage_options' ) ) {
			$message .= ' ' . sprintf(
					...
					__( 'Go to the %1$sAccelerateWP settings%2$s page to track progress.', 'rocket' ),
	...
	public function init_critical_css_generation() {
		...
		if ( ! strpos( wp_get_referer(), 'clsop' ) ) {
	...
	public function critical_css_generation_complete_notice() {
		...
		if ( 'error' === $status || 'warning' === $status ) {
			$message .= '<p>' . __( 'Critical CSS generation encountered one or more errors.', 'rocket' ); /*. ' <a href="https://docs.cloudlinux.com/article/1267-troubleshooting-critical-css-generation-issues" data-beacon-article="5d5214d10428631e94f94ae6" target="_blank" rel="noreferer noopener">' . __( 'Learn more.', 'rocket' ) . '</a>'*/
		}
...
JS;
	...
	/**
	 * Stops the critical CSS generation when deactivating the async CSS option via CLI and remove the notices.
	 *
	 * @since 3.12.6
	 */
	public function stop_process_on_cli_deactivation() {
		$this->stop_critical_css_generation();
	}
	 ...
	 * @param \WP_Rocket\Engine\Admin\Settings\Settings $settings Settings class instance.
	 *
	 * @note CL
	 * @return array
	 */
	public function sanitize_options( $input, $settings ) : array {
		...
		$cpcss_enabled = (bool) $this->options->get( 'async_css', 0 );
		if ( $cpcss_enabled ) {
			$input['ccss_awp_unique_id']    = $this->options->get( 'ccss_awp_unique_id', false );
			$input['async_css']             = 1;
			$input['async_css_mobile']      = 1;
			$input['remove_unused_css']     = 0;
			$input['optimize_css_delivery'] = '1';
		}

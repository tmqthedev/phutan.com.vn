...
function rocket_warning_footer_js_plugin() {
	...
	rocket_notice_html(
		[
			'status'         => 'warning',
			'message'        => __( 'AccelerateWP Footer JS is not an official add-on. It prevents some options in AccelerateWP from working correctly. Please deactivate it if you have problems.', 'rocket' ),
			'dismiss_button' => true,
		]
	);
}
	...
	rocket_notice_html(
		[
			...
			'message' => sprintf(
				...
				__( 'Endurance Cache is currently enabled, which will conflict with AccelerateWP Cache. Please set the Endurance Cache cache level to Off (Level 0) on the %1$sSettings > General%2$s page to prevent any issues.', 'rocket' ),
...
function rocket_warning_htaccess_permissions() {
	...
	$message .= '<br>' . sprintf(
		...
		'<a href="' . __( 'https://docs.cloudlinux.com/article/626-how-to-make-system-files-htaccess-wp-config-writeable/?utm_source=wp_plugin&utm_medium=cloudlinux_site_optimization_plugin', 'rocket' ) . '" target="_blank">',
		'</a>'
	);
	*/
...
function rocket_thank_you_license() {
	if ( '1' === get_rocket_option( 'license' ) ) {
		...
		$message = sprintf(
			...
			__( '%1$s is good to go! Visit your %3$ssettings%4$s.', 'rocket' ),
			'<strong>' . WP_ROCKET_PLUGIN_NAME . '</strong>',
			'<a href="https://cloudlinux.com/blog/how-to-test-wordpress-site-performance-measure-speed-results/?utm_source=wp_plugin&utm_medium=cloudlinux_site_optimization_plugin" target="_blank">',
...
function rocket_analytics_optin_notice() {
	...
	$analytics_notice = sprintf(
		...
		__( 'Would you allow AccelerateWP to collect non-sensitive diagnostic data from this website?', 'rocket' ),
		__( 'This would help us to improve AccelerateWP for you in the future.', 'rocket' )
	);
	...
	$analytics_notice .= sprintf(
		...
		__( 'Below is a detailed view of all data AccelerateWP will collect if granted permission. AccelerateWP will never transmit any domain names or email addresses (except for license validation), IP addresses, or third-party API keys.', 'rocket' ),
...
function rocket_analytics_optin_thankyou_notice() {
	...
	$thankyou_message .= sprintf(
		'<p>%1$s</p><div>%2$s</div>',
		__( 'AccelerateWP now collects these metrics from your website:', 'rocket' ),
		rocket_data_collection_preview_table()
	);
...
function rocket_notice_html( $args ) {
	...
	switch ( $args['action'] ) {
		case 'clear_cache':
			$args['action'] = '<a class="wp-core-ui button" href="' . wp_nonce_url( admin_url( 'admin-post.php?action=purge_cache&type=all' . \WP_Rocket\Buffer\Cache::get_referrer() ), 'purge_cache_all' ) . '">' . __( 'Clear cache', 'rocket' ) . '</a>';
			break;
		case 'clear_used_css':
			$params = [
		...
		case 'force_deactivation':
			...
			break;
		case 'clsop_link':
			$args['action'] = '<a class="wp-core-ui button" href="' . wp_nonce_url( admin_url( $args['action_link_url'] ), $args['action_nonce'] ) . '">' . $args['action_link_text'] . '</a>';
			break;
	}
...
/**
 * Display notice from RPM package notices.json
 *
 * @note CL
 * @since 3.10.4
 */
function clsop_warning_message() {
	if ( isset( $_GET['action'] ) && 'upgrade-plugin' === $_GET['action'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	if ( current_user_can( 'rocket_manage_options' ) ) {
		...
		$clsop_notices_file = rocket_get_constant( 'WP_ROCKET_UPDATE_PATH' ) . DIRECTORY_SEPARATOR . 'notices.json';

		if ( @is_readable( $clsop_notices_file ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
			$clsop_notices_content = file_get_contents( $clsop_notices_file ); //phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents

			$clsop_notices = ( $clsop_notices_content ) ? json_decode( $clsop_notices_content, true ) : $clsop_notices_content;
		}

		if ( ! isset( $clsop_notices ) || ! is_array( $clsop_notices ) ) {
			return;
		}

		// Ignoring all notices with versions higher than the plugin version.
		foreach ( $clsop_notices as $notice_key => $notice ) {
			if ( version_compare( rocket_get_constant( 'WP_ROCKET_VERSION' ), $notice['maximum_display_version'], '>' ) ) {
				unset( $clsop_notices[ $notice_key ] );
			}
		}

		// Retrieve notice with minimal version.
		if ( ! empty( $clsop_notices ) ) {
			$notice = array_reduce(
				$clsop_notices,
				function ( $first_notice, $second_notice ) {
					if ( ! $first_notice ) {
						return $second_notice;
					}
					return (
						version_compare(
							$first_notice['maximum_display_version'],
							$second_notice['maximum_display_version'],
							'>'
							)
		...
		rocket_notice_html(
			[
				'status'           => 'error',
				'dismissible'      => '',
				'message'          => '<strong>' . rocket_get_constant( 'WP_ROCKET_PLUGIN_NAME' ) . '</strong>: ' . $notice['text'],
				'action'           => 'clsop_link',
				'dismiss_button'   => __FUNCTION__,
				'action_link_url'  => $notice['action_link_url'],
				'action_nonce'     => $notice['action_nonce'],
				'action_link_text' => $notice['action_link_text'],
			]
		);
	}
}
add_action( 'admin_notices', 'clsop_warning_message' );

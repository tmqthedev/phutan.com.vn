...
function get_rocket_documentation_url() { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals
	...
	$url    = 'https://user-docs.cloudlinux.com/wpos-plugin/#site-optimization-plugin'; // CL.
 ...
 * @author Remy Perona
 *
 * @return void URL in the correct language
 */
function get_rocket_faq_url() { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals
	...
	$url    = WP_ROCKET_WEB_MAIN . "{$lang}faq/?utm_source=wp_plugin&utm_medium=cloudlinux_site_optimization_plugin";

	/*
	// Temporary off external urls
	return $url;
	*/
}
...
function rocket_data_collection_preview_table() {
	...
	$html .= '<tr>';
	$html .= '<td class="column-primary">';
	$html .= sprintf( '<strong>%s</strong>', __( 'Anonymized AccelerateWP settings:', 'rocket' ) );
	$html .= '</td>';
	$html .= '<td>';
	$html .= sprintf( '<em>%s</em>', __( 'Which AccelerateWP settings are active', 'rocket' ) );
	$html .= '</td>';
	$html .= '</tr>';
	...
	$html .= '<tr>';
	$html .= '<td class="column-primary">';
	$html .= sprintf( '<strong>%s</strong>', __( 'AccelerateWP license type', 'rocket' ) );
	$html .= '</td>';
	$html .= '<td>';
...
/**
 * Dismiss one box for all administrators.
 *
 * @since 3.10.4
 * @note CL
 * @param string $func Function (box) name.
 */
function clsop_dismiss_box_for_all( $func ) {
	$wp_user_query = new WP_User_Query( [ 'role' => 'Administrator' ] );
	$users         = $wp_user_query->get_results();

	if ( ! empty( $users ) ) {
		foreach ( $users as $user ) {
			$actual = get_user_meta( $user->ID, 'rocket_boxes', true );
			$actual = array_merge( (array) $actual, [ $func ] );
			$actual = array_filter( $actual );
			$actual = array_unique( $actual );

			update_user_meta( $user->ID, 'rocket_boxes', $actual );
			delete_transient( $func );
		}
	}
}

...
class Subscriber implements Subscriber_Interface {
	...
	public static function get_subscribed_events() {
		...
		return [
			...
			'rocket_purge_cloudflare_api'               => 'purge_cloudflare',
	...
	public function purge_cache_no_die() {
		...
		if ( is_wp_error( $connection ) ) {
			$cf_purge_result = [
				...
				'message' => sprintf(
					...
					__( '%1$sAccelerateWP:%2$s %3$s', 'rocket' ),
					'<strong>',
					'</strong>',
		...
		$cf_purge_result = [
			...
			'message' => sprintf(
				...
				__( '%1$sAccelerateWP:%2$s Cloudflare cache successfully purged.', 'rocket' ),
				'<strong>',
				'</strong>'
			),
		];
	...
	private function save_cloudflare_devmode( $value ) {
		...
		if ( is_wp_error( $result ) ) {
			return [
				...
				'message' => '<strong>' . __( 'AccelerateWP: ', 'rocket' ) . '</strong>' . sprintf( __( 'Cloudflare development mode error: %s', 'rocket' ), $cloudflare_dev_mode_return->get_error_message() ),
			];
		}
		...
		return [
			'result'  => 'success',
			// translators: %s is the message returned by the CloudFlare API.
			'message' => '<strong>' . __( 'AccelerateWP: ', 'rocket' ) . '</strong>' . sprintf( __( 'Cloudflare development mode %s', 'rocket' ), $cloudflare_dev_mode_return ),
		];
	}
	...
	public function save_cloudflare_options( $old_value, $value ) {
		...
		$result = [
			'pre' => sprintf(
				...
				__( '%1$sAccelerateWP:%2$s Optimal settings activated for Cloudflare:', 'rocket' ),
				'<strong>',
				'</strong>'
			) . '<br>',
		];
		...
		if ( 0 === (int) $value['cloudflare_auto_settings'] ) {
			$result['pre'] = sprintf(
				// translators: %1$s = strong opening tag, %2$s = strong closing tag.
				__( '%1$sAccelerateWP:%2$s Optimal settings deactivated for Cloudflare, reverted to previous settings:', 'rocket' ),
				'<strong>',
				'</strong>'
			) . '<br>';
		}
	...
	public function display_settings_notice( $value, $old_value ) {
		...
		if ( is_wp_error( $connection ) ) {
			add_settings_error( 'general', 'cloudflare_api_key_invalid', __( 'AccelerateWP: ', 'rocket' ) . '</strong>' . $connection->get_error_message() . '<strong>', 'error' );
		}

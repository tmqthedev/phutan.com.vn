	...
	if ( ! isset( $cf_email, $cf_api_key ) || empty( $cf_email ) || empty( $cf_api_key ) ) {
		return new WP_Error(
			...
			/* translators: %1$s = opening link; %2$s = closing link */
				...
				'<a href="' . esc_url( __( 'https://docs.cloudlinux.com/article/18-using-wp-rocket-with-cloudflare/?utm_source=wp_plugin&utm_medium=cloudlinux_site_optimization_plugin#add-on', 'rocket' ) ) . '" rel="noopener noreferrer" target="_blank">',
				'</a>'
			)
		);
	}
...
function rocket_set_real_ip_cloudflare() {
	...
	if ( ! headers_sent() ) {
		header( 'X-CF-Powered-By: AccelerateWP ' . WP_ROCKET_VERSION );
	}
}
...
function do_admin_post_rocket_purge_cloudflare() {
	...
	if ( is_wp_error( $cf_purge ) ) {
		$cf_purge_result = [
			...
			'message' => sprintf( __( '<strong>AccelerateWP:</strong> %s', 'rocket' ), $cf_purge->get_error_message() ),
		];
	} else {
		$cf_purge_result = [
			'result'  => 'success',
			'message' => __( '<strong>AccelerateWP:</strong> Cloudflare cache successfully purged.', 'rocket' ),
		];
	}
...
function rocket_warning_cron() {
	...
	$message = '<p>' . _n( 'The following scheduled event failed to run. This may indicate the CRON system is not running properly, which can prevent some AccelerateWP features from working as intended:', 'The following scheduled events failed to run. This may indicate the CRON system is not running properly, which can prevent some AccelerateWP features from working as intended:', count( $events ), 'rocket' ) . '</p>';
...
function rocket_tag_row_actions( $actions, $term ) {
	...
	$url                     = wp_nonce_url( admin_url( 'admin-post.php?action=purge_cache&type=term-' . $term->term_id . '&taxonomy=' . $taxnow . \WP_Rocket\Buffer\Cache::get_referrer() ), 'purge_cache_term-' . $term->term_id );

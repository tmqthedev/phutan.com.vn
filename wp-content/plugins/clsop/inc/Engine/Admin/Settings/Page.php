...
class Page {
	...
	private function dashboard_section() {
		$this->settings->add_page_section(
			...
			[
				...
				'menu_description' => __( 'Quick actions', 'rocket' ),
		...
		$this->settings->add_settings_fields(
			[
				'analytics_enabled' => [
					...
					'description'       => sprintf( __( 'I agree to share anonymous data with the development team to help improve AccelerateWP. %1$sWhat info will we collect?%2$s', 'rocket' ), '<button class="wpr-js-popin">', '</button>' ),
					'section'           => 'status',
					'page'              => 'dashboard',
					'default'           => 0,
		...
		$this->settings->add_settings_sections(
			[
				...
				'user_cache_section'   => [
					...
					'type'        => 'fields_container',
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					'description' => sprintf( __( 'User cache is great when you have user-specific or restricted content on your website.', 'rocket' ), '<a href="' . esc_url( $user_cache_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $user_cache_beacon['id'] ) . '" target="_blank">', '</a>' ),
					'help'        => [
	...
	private function assets_section() {
		...
		*/
		...
		if ( get_rocket_option( 'optimize_css_delivery', false ) ) {
			$this->settings->add_settings_fields(
				[
					'optimize_css_delivery' => [
						...
						'description'       => esc_html__( 'Optimize CSS delivery eliminates render-blocking CSS on your website.', 'rocket' ) . '<br>' . self::get_disable_feature_text( 'cpcss' ),
						'section'           => 'css',
						'page'              => 'file_optimization',
						'default'           => 0,
						'sanitize_callback' => 'sanitize_checkbox',
						'input_attr'        => [
							'disabled' => 1, // $disable_ocd ? 1 : 0,
						],
						...
						'options'                 => [
							...
							'async_css' => [
								...
								'description' => is_plugin_active( 'wp-criticalcss/wp-criticalcss.php' ) ?
									...
									// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
									sprintf( __( 'Generates critical path CSS and loads CSS asynchronously, 'rocket' ), '<a href="' . esc_url( $async_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $async_beacon['id'] ) . '" target="_blank">', '</a>' ),
		...
		$this->settings->add_settings_sections(
			[
				'lazyload_section'     => [
					...
					'helper'      => ! empty( $disable_lazyload ) ? sprintf( __( 'LazyLoad is currently activated in %2$s. If you want to use AccelerateWP LazyLoad, disable this option in %2$s.', 'rocket' ), WP_ROCKET_PLUGIN_NAME, $disable_lazyload ) : '',
				],
				'dimensions_section'   => [
		...
		$this->settings->add_settings_fields(
			[
				...
				'awp_image_optimization' => [
					...
					'description'       => get_rocket_option( 'awp_image_optimization' )
						?
		...
		$this->settings->add_settings_sections(
			[
				...
				'cache_query_strings_section'  => [
					...
					'type'        => 'fields_container',
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					'description' => sprintf( __( 'Cache for query strings enables you to force caching for specific GET parameters.', 'rocket' ), '<a href="' . esc_url( $cache_query_strings_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $cache_query_strings_beacon['id'] ) . '" target="_blank">', '</a>' ),
		...
		$this->settings->add_settings_sections(
			[
				'cdn_section'         => [
					'title'       => __( 'CDN', 'rocket' ),
					'type'        => 'fields_container',
					'description' => __( 'All URLs of static files (CSS, JS, images) will be rewritten to the CNAME(s) you provide.', 'rocket' ) . '<br>',
	...
	private function addons_section() {
		// CL.
		if ( ! get_rocket_option( 'addon_varnish_used' ) && ! get_rocket_option( 'addon_webp_used' ) && ! get_rocket_option( 'addon_cloudflare_used' ) ) {
			return;
		}
		...
		// CL.
		if ( get_rocket_option( 'addon_varnish_used' ) || get_rocket_option( 'addon_webp_used' ) ) {
			$this->settings->add_settings_sections(
				[
					'one_click' => [
						'title'       => __( 'One-click AccelerateWP Add-ons', 'rocket' ),
		...
		*/
				...
				apply_filters(
					...
					[
						'varnish_auto_purge' => [
							...
							// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
							'description'       => sprintf( __( 'Varnish cache will be purged each time AccelerateWP clears its cache to ensure content is always up-to-date.'/*<br>%1$sLearn more%2$s*/, 'rocket' ), '<a href="' . esc_url( $varnish_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $varnish_beacon['id'] ) . '" target="_blank">', '</a>' ),
							'section'           => 'one_click',
							'page'              => 'addons',
						...
						apply_filters(
							...
							[
								...
								// translators: %1$s and %3$s = opening <a> tag, %2$s = closing </a> tag.
									__( 'Enable this option if you would like AccelerateWP to serve WebP images to compatible browsers. Please note that AccelerateWP cannot create WebP images for you.'/* To create WebP images we recommend %1$sImagify%2$s. %3$sMore info%2$s*/, 'rocket' ),
									$imagify_link,
									'</a>',
		...
		if ( defined( 'WP_ROCKET_SUCURI_API_KEY_HIDDEN' ) && WP_ROCKET_SUCURI_API_KEY_HIDDEN ) {
			...
			$description   = __( 'Clear the Sucuri cache when AccelerateWP cache is cleared.', 'rocket' );
			$settings_page = false;
		} else {
			$description   = __( 'Provide your API key to clear the Sucuri cache when AccelerateWP cache is cleared.', 'rocket' );
	...
	public static function get_enable_feature_text( $feature_slug ) {
		...
		if ( strlen( $smart_advice_link ) > 0 ) {
			return sprintf(
				...
				'<a href="' . esc_url( $smart_advice_link ) . '">',
				'</a>'
			);
		}

...
class WPRocketUninstall {
	 ...
	 * @param string  $wp_content_dir        Path to wp-content dir.
	 */
	public function __construct( $cache_path, $config_path, $rucss_usedcss_table, $rocket_cache, $wp_content_dir ) {
	...
	public function uninstall() {
		...
		$this->delete_advanced_cache_file(); // CL.
	...
	/**
	 * Deleting /wp-content/advanced-cache.php file
	 *
	 * @note CL
	 * @return void
	 */
	private function delete_advanced_cache_file() {
		$advanced_cache_file = $this->wp_content_dir . '/advanced-cache.php';
		if ( is_readable( $advanced_cache_file ) ) {
			if ( empty( file_get_contents( $advanced_cache_file ) ) ) { //phpcs:ignore
				@unlink( $advanced_cache_file ); //phpcs:ignore WordPress.WP.AlternativeFunctions.unlink_unlink, WordPress.PHP.NoSilencedErrors.Discouraged
			}
		}
	}
}

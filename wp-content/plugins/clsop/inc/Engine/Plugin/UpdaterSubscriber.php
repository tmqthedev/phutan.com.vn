...
class UpdaterSubscriber implements Event_Manager_Aware_Subscriber_Interface {
	...
	/**
	 * Path to contact to get update info.
	 *
	 * @note CL
	 * @var string
	 */
	 ...
	 * }
	 */
	public function __construct( RenewalNotice $renewal_notice, ApiSocket $api_socket, $args ) {
		$this->api_socket = $api_socket;

		foreach ( [ 'plugin_file', 'plugin_version', 'vendor_url', 'local_path', 'icons' ] as $setting ) {
	...
	public static function get_subscribed_events() {
		return [
			...
			'upgrader_pre_download'                    => [ 'get_download_link', 10, 2 ],
		];
	}
	...
	public function get_latest_version_data() {
		...
		if (
			rocket_get_constant( 'WP_ROCKET_VERSION' ) !== null
			&&
			version_compare( rocket_get_constant( 'WP_ROCKET_VERSION' ), $obj->new_version, '<' )
			) {
			rocket_renew_box( 'clsop_warning_message' );
		}
	...
	/**
	 * Prepare pacage link for installation process.
	 * 
	...
	public function rollback() {
		...
		$plugin_transient->response[ $plugin ] = (object) [
			...
			'package'     => sprintf( 'https://cloudlinux.com/%s/wp-rocket_%s.zip', get_rocket_option( 'consumer_key' ), WP_ROCKET_LASTVERSION ),
		];

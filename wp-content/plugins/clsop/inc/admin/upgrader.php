...
function rocket_upgrader() {
	...
	if ( did_action( 'wp_rocket_first_install' ) || did_action( 'wp_rocket_upgrade' ) ) {
		...
		// CL.
		rocket_renew_all_boxes( 0, [ 'clsop_warning_message' ] );
	...
	if (
		...
		wp_safe_redirect( esc_url_raw( admin_url( 'options-general.php?page=clsop' ) ) );
		exit;
	}

	if ( ! rocket_valid_key() && current_user_can( 'rocket_manage_options' ) && 'clsop' === $page ) {
...
function rocket_first_install() {
	...
	clsop_dismiss_box_for_all( 'rocket_warning_plugin_modification' );
	clsop_dismiss_box_for_all( 'clsop_warning_message' );
}
...
function rocket_new_upgrade( $wp_rocket_version, $actual_version ) {
	...
	// CL.
	if ( version_compare( $actual_version, '3.12.3.2', '<' ) ) {
		$options = get_option( rocket_get_constant( 'WP_ROCKET_SLUG' ) );
		...
		$options['addon_varnish_used']    = array_key_exists( 'varnish_auto_purge', $options ) ? (int) $options['varnish_auto_purge'] : 0;
		$options['addon_webp_used']       = array_key_exists( 'cache_webp', $options ) ? (int) $options['cache_webp'] : 0;
		$options['addon_cloudflare_used'] = array_key_exists( 'do_cloudflare', $options ) ? (int) $options['do_cloudflare'] : 0;

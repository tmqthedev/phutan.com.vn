...
function rocket_settings_action_links( $actions ) {
	...
	/*
	//Temporary off external links
	array_unshift( $actions, sprintf( '<a href="%s">%s</a>', 'https://cloudlinux.com/support/?utm_source=wp_plugin&utm_medium=cloudlinux_site_optimization_plugin', __( 'Support', 'rocket' ) ) );
...
function rocket_plugin_row_meta( $plugin_meta, $plugin_file ) {
	if ( 'cloudlinux-site-optimization-plugin/wp-rocket.php' === $plugin_file ) {
		...
		if ( false !== $update_plugins && isset( $update_plugins->response[ $plugin_file ] ) && empty( $update_plugins->response[ $plugin_file ]->package ) ) {
			...
			$link = '<span class="dashicons dashicons-update rocket-dashicons"></span> <span class="rocket-renew">Renew your licence of AccelerateWP to receive access to automatic upgrades and support.</span> <a href="http://cloudlinux.com" target="_blank" class="rocket-purchase">Purchase now</a>.';
...
function rocket_user_row_actions( $actions, $user ) {
	...
	$url                     = wp_nonce_url( admin_url( 'admin-post.php?action=purge_cache&type=user-' . $user->ID . \WP_Rocket\Buffer\Cache::get_referrer() ), 'purge_cache_user-' . $user->ID );
	$actions['rocket_purge'] = sprintf( '<a href="%s">%s</a>', $url, __( 'Clear this cache', 'rocket' ) );
...
function rocket_send_analytics_data() {
	...
	/*
	// Turn off sending analytics data

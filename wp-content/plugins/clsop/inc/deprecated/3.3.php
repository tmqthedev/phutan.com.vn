...
if ( ! function_exists( 'get_rocket_footprint' ) ) :
	...
	function get_rocket_footprint( $debug = true ) {
		...
		$footprint = defined( 'WP_ROCKET_WHITE_LABEL_FOOTPRINT' ) ?
						...
						"\n" . '<!-- Performance optimized by ' . WP_ROCKET_PLUGIN_NAME .'.' /*. '. Learn more: https://cloudlinux.com'*/;
		if ( $debug ) {
	...
	function rocket_force_info_result( $res, $action, $args ) {
		...
		if ( 'plugin_information' === $action && isset( $args->slug ) && 'wp-rocket' === $args->slug && isset( $res->external ) && $res->external ) {
			...
			if ( is_wp_error( $request ) ) {
				...
				$res = new WP_Error( 'plugins_api_failed', sprintf( __( 'An unexpected error occurred. Something may be wrong with external or this server&#8217;s configuration.'/* If you continue to have problems, <a href="%s">contact support</a>.*/, 'rocket' ), rocket_get_external_url( 'support', array(
...
if ( ! function_exists( 'rocket_check_update' ) ) :
	...
	function rocket_check_update( $value ) {
		...
		$timer_update_clsop = (int) get_site_transient( 'update_clsop' );
		$temp_object           = get_site_transient( 'update_clsop_response' );
		if ( ( ! isset( $_GET['rocket_force_update'] ) || defined( 'WP_INSTALLING' ) ) &&
			( 12 * HOUR_IN_SECONDS ) > ( time() - $timer_update_clsop ) // retry in 12 hours.
		) {
			if ( is_object( $value ) && false !== $temp_object ) {
				if ( version_compare( $temp_object->new_version, WP_ROCKET_VERSION ) > 0 ) {
					...
					delete_site_transient( 'update_clsop_response' );
				}
			}
			return $value;
		}
		...
		if ( ! is_a( $response, 'WP_Error' ) && 200 === $response['response']['code'] && strlen( $response['body'] ) > 32 ) {
			...
			set_site_transient( 'update_clsop', time() );
			...
			set_site_transient( 'update_clsop_response', $temp_object );
		} else {
			set_site_transient( 'update_clsop', ( time() + ( 11 * HOUR_IN_SECONDS ) ) ); // retry in 1 hour in case of error..
		}
		return $value;
	}
endif;
	...
	function rocket_reset_check_update_timer( $transient = 'update_plugins', $value = null ) {
		...
		if ( 'update_plugins' === $transient ) {
			if ( is_null( $value ) || is_object( $value ) && ! isset( $value->response ) ) {
				delete_site_transient( 'update_clsop' );
			}
		}
	}
endif;

...
class Cloudflare implements Subscriber_Interface, DeactivationInterface {
	...
	public function display_apo_cookies_notice() {
		...
		__( '%1$sAccelerateWP:%2$s You are using "Dynamic Cookies Cache". Cloudflare APO is not yet compatible with that feature.', 'rocket' ) . '<br>',
		'<strong>',
			'</strong>'
		);
		$message .= sprintf(
		...
		if (
			...
			rocket_notice_html(
				[
					...
					'message'     => sprintf(
						...
						__( '%1$sAccelerateWP:%2$s You are using "Separate cache files for mobile devices". You need to activate "Cache by Device Type" %3$ssetting%5$s on Cloudflare APO to serve the right version of the cache. %4$sMore info%5$s', 'rocket' ),
						'<strong>',
						'</strong>',
			...
			rocket_notice_html(
				[
					...
					'message'        => sprintf(
					// Translators: %1$s = strong opening tag, %2$s = strong closing tag.
						__( '%1$sAccelerateWP:%2$s You have "Cache by Device Type" enabled on Cloudflare APO. If you judge it necessary for the website to have a different cache on mobile and desktop, we suggest you enable our “Separate Cache Files for Mobiles Devices” to ensure the generated cache is accurate.', 'rocket' ),
						'<strong>',
						'</strong>'
					),
